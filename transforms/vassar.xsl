...
-->
	...
	<xsl:template match="mods:name">
		<xsl:choose>
<!-- jd changed to add case-insensitivity for creator designation -->
			<xsl:when
				test="mods:role/mods:roleTerm[@type='text']='creator' or mods:role/mods:roleTerm[@type='text']='Creator' or mods:role/mods:roleTerm[@type='code']='cre' ">
				<dc:creator>
...
-->
<!-- this is for dc:date mappintg to dateOther -->
<!--
		<xsl:for-each
...
<!-- jd adding additional template to handle dateOther and match it to DC temporal coverage -->
<!-- commenting out: better idea is to keep dateOther as part of dc.date, since dc.date is so imprecise anyway -->
<!--
		<xsl:for-each select="mods:dateOther">
			<dc:coverage>
				<xsl:value-of select="." />
